...
def create_mobility_demand_v2(n_veichles, dict_tile_edges, od_matrix, road_network, filename,
                           timing="start", time_range=(0, 3601), random_seed=None, cache_path_validity={},
                           show_progress=False, show_warnings=True, vehicles_background=None, threshold_km=1,
                           background_suffix="background_", vehicle_suffix="vehicle_", allow_self_tiles=True):
    
    departure_times = []
    
    print("version 2.0")
    print(f"# keys into cache dict: {len(cache_path_validity)}")
    
    # select n_veichles OD pairs from the od-matrix
    od_pairs, choices, cache_path_validity = create_traffic_from_matrix_v2(od_matrix, dict_tile_edges, n_veichles, road_network, 
                                                threshold_km=threshold_km, allow_self_tiles=False,
                                                show_progress=show_progress, show_warnings=show_warnings, random_seed=random_seed,
                                                cache_path_validity=cache_path_validity)
    ...
    return od_pairs, departure_times, dict_mobility_demand_pairs, cache_path_validity
...
def create_traffic_from_matrix_v2(od_matrix, dict_tile_edges, n_vehicles, road_network, threshold_km=1.2, 
                                  random_seed=None, allow_self_tiles=True, show_progress=True, show_warnings=True,
                                  cache_path_validity={}):
    ...
    # discard tile with one edge
    
    for k in dict_tile_edges: 
        if len(dict_tile_edges[k])<=1:
            #print(k)
            od_matrix[int(k)] = 0
            od_matrix[:, int(k)] = 0
    ...
    return od_edges_list, choices_list, cache_path_validity



def init_traci(warnings=True):
    ...
    if warnings:
        sumo_cmd = [sumo_binary, "-c", "../data/simulations/config_init_traci.sumocfg"]
    else:
        sumo_cmd = [sumo_binary, "-c", "../data/simulations/config_init_traci.sumocfg", '--no-warnings']
...
def has_valid_route_traci_v2(edge_list, dict_lookup, road_network, dist_th):
    
    e_from, e_to = edge_list[0], edge_list[-1]
    
    str_key = f"{e_from}_AND_{e_to}"
    
    if str_key in dict_lookup:
        return dict_lookup[str_key]
    

    for i in range(len(edge_list)-1):
        
        e_0 = road_network.getEdge(edge_list[i])
        e_1 = road_network.getEdge(edge_list[i+1])
        
        e_list = traci.simulation.findRoute(edge_list[i], edge_list[i+1]).edges
        #e_list = []
        
        #res_sumo = road_network.getOptimalPath(e_0, e_1, fastest=True)
        
        #print(res_sumo)
        
        #if res_sumo[0] is None:
        #    e_list = []
        #else:
        #    for e in res_sumo[0]:
        #        e_list.append(e.getID())

        if len(e_list)==0:
            dict_lookup[str_key] = False
            return False
        
        if compute_length_path_km(e_list, road_network) < dist_th:
            dict_lookup[str_key] = False
            #print("less th")
            return False
    
    dict_lookup[str_key] = True
    
    return True


def compute_length_path_km(e_list, road_network):
    
    tot_len_m = 0

    for e in e_list:
        tot_len_m+=road_network.getEdge(e).getLength()
        
    return tot_len_m/1000
